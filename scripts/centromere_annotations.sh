...
# extract centromeres from Sbay_revised.gff
awk -F"\t" '/CEN/ {OFS="\t"; print "Sbay_" $1, $4, $5}' $refs/Sbay.gff > $refs/Sbay_cen.gff

# extract centromeres from Sbay_revised.gff
awk -F"\t" '/CEN/ {OFS="\t"; print "Sbay_" $1, $4, $5}' $refs/Sbay_revised.gff > $refs/Sbay_revised_cen.gff
...
bowtie2 -f --very-sensitive -x $bt2/Y12_quiver -U $refs/Scer_cen.fa | \
  grep Y12 | grep -v '^@' | cut -f3,4,6 | \
  sed 's/M/+/g' | sed 's/D/+/g' | sed 's/[0-9]*I//' | sed 's/$/0/' | \
  while read a b c; do echo $a $b $(($c)); done | \
  awk '{OFS="\t"; print $1, $2, $2+$3}' > $refs/Y12_quiver_cen.gff
bowtie2 -f --very-sensitive -x $bt2/DBVPG6044_quiver -U $refs/Scer_cen.fa | \
  grep DBVPG6044 | grep -v '^@' | cut -f3,4,6 | \
  sed 's/M/+/g' | sed 's/D/+/g' | sed 's/[0-9]*I//' | sed 's/$/0/' | \
  while read a b c; do echo $a $b $(($c)); done | \
  awk '{OFS="\t"; print $1, $2, $2+$3}' > $refs/DBVPG6044_quiver_cen.gff
